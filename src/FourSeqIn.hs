...
-- main function is to convert sam file to fasta, which contains trimmed unknow sequences, for mapping
...
readCount = length
readAveLength x = (/) (fromIntegral $ sum $ map (T.length . samseq) x) (L.genericLength x)
readStats x = (readCount x, readAveLength x)
...
getUnknownSeqToFasta re1 re2 r = map reconstructReads qualifiedReads
  where qualifiedReads = filterReadsWithBothEnzymes r re1 re2
        reconstructReads r = Fasta (T.concat [">", qname r]) (newseq r)
